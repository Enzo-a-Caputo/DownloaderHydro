...
Migrado de `delineator-main/py/merit_detailed.py` (Matthew Heberger,
mghydro.com/watersheds). Mudanças:
- removido `from config import *` e `from py.raster_plots import *` (sys.path
  injection eliminado); todas as configs agora vêm como parâmetros da função
- corrigido `import numpy as np` que antes vinha por transitividade do
  `from py.raster_plots import *`
- corrigido o caso de erro pós-snap (linha 233 original) onde a ordem do
  retorno era (None, lng_snap, lat_snap), inconsistente com o caminho feliz
  (poly, lat_snap, lng_snap); agora todos os returns seguem (poly, lat, lng)
"""
import os
...
from .raster_plots import (
    plot_mask, plot_flowdir, plot_accum, plot_streams, plot_catchment,
    plot_clipped, plot_polys,
)
...
def split_catchment(
    ...
    *,
    ...
    Args:
        ...
        merit_fdir_dir: directory containing the basin-scale MERIT-Hydro flow direction rasters
            (`flowdir<basin>.tif`). No trailing slash.
        merit_accum_dir: directory containing the basin-scale MERIT-Hydro flow accumulation rasters
            (`accum<basin>.tif`).
        threshold_single: minimum upstream pixels to define a stream when the watershed
            consists of a single unit catchment. Smaller value catches headwater streams.
        threshold_multiple: same, but when the watershed has multiple unit catchments.
            Larger value avoids weird topology near confluences.
        verbose: if True, prints progress messages.
        plots: if True, writes debug plots into ./plots/ (folder must exist).

    Returns:
        (poly, lat_snap, lng_snap):
        - poly: shapely polygon representing the part of the terminal unit catchment
                upstream of the outlet point. None if pysheds failed.
        - lat_snap: latitude of the outlet, snapped to the river centerline. None if snap failed.
        - lng_snap: longitude of the outlet, snapped to the river centerline. None if snap failed.
    ...
    if snap:
        ...
        # Sem snap: usar o ponto exato do usuário como exutório.
        # O raster de acumulação já foi lido (necessário para grid.catchment abaixo)
        # mas não buscamos o rio mais próximo — delineamos a partir do pixel clicado.
        if verbose: print("Snap desativado — usando ponto exato ({}, {})".format(lat, lng))
    ...
    try:
        ...
        # Mantém ordem (poly, lat, lng) — original retornava (None, lng, lat) aqui, bug.
        return None, lat_snap, lng_snap
